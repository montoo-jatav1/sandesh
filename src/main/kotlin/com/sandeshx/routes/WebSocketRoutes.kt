...
private data class WsIncoming(
    val type: String, // "message" | "read" | "typing" | "edit" | "delete" | "group_message"
...
@Serializable
private data class WsGroupMessageOut(
    val type: String = "group_message",
    val id: Long,
    val groupId: Long,
    val senderId: Long,
    val body: String? = null,
            ...
            for (frame in incoming) {
                ...
                when (event.type) {
                    "message" -> {
                        ...
                        // Echo to sender + push to receiver FIRST, before any other
                        // bookkeeping — this is the part latency actually matters for.
                        // (Previously `unarchiveOnNewMessage` ran as a blocking DB call
                        // right here, ahead of the push, adding a full extra round-trip
                        // of latency to every single message before anyone saw it.)
                        ...
                        // Everything below is bookkeeping that nobody is waiting on —
                        // run it after the push, not before, and in the background so
                        // it can't add latency to the *next* message either.
                        launch { com.sandeshx.services.ChatPreferencesService.unarchiveOnNewMessage(receiverId, userId) }
                        if (receiverSessions.isNotEmpty()) {
                            launch { MessageService.markDelivered(saved.id) }
                        } else {
                            launch {
                    ...
                    "group_message" -> {
                        val groupId = event.groupId ?: continue
                        val saved = try {
                            com.sandeshx.services.GroupService.send(groupId, userId, event.body, event.imageUrl)
                        } catch (e: Exception) {
                            continue // not a member, or empty message — silently drop rather than crash the socket
                        }
                        val payload = Json.encodeToString(
                            WsGroupMessageOut.serializer(),
                            WsGroupMessageOut(
                                id = saved.id,
                        ...
                        val memberIds = com.sandeshx.services.GroupService.memberUserIds(groupId)
                        for (memberId in memberIds) {
                            ConnectionRegistry.sessionsFor(memberId).forEach { it.send(Frame.Text(payload)) }
                        }
                    }
                }
            }
