...
fun Route.chatRoutes() {
    authenticate("auth-jwt") {
        route("/api/chats") {
            ...
            // "Mark all as read" from the chat list overflow menu.
            post("/mark-all-read") {
                val me = call.currentUserId()
                val senderIds = MessageService.markAllRead(me)
                val payload = kotlinx.serialization.json.Json.encodeToString(
                    kotlinx.serialization.json.JsonObject.serializer(),
                    kotlinx.serialization.json.buildJsonObject {
                        put("type", kotlinx.serialization.json.JsonPrimitive("bulk_read"))
                        put("readerId", kotlinx.serialization.json.JsonPrimitive(me))
                    }
                )
                for (senderId in senderIds) {
                    com.sandeshx.services.ConnectionRegistry.sessionsFor(senderId).forEach { session ->
