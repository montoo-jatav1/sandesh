...
object Groups : LongIdTable("groups") {
    ...
    val ownerId = long("owner_id").references(Users.id)
    val avatarUrl = varchar("avatar_url", 500).nullable()
    /** Short random code used for both the shareable invite link and the QR code. */
    val inviteCode = varchar("invite_code", 20).uniqueIndex()
    val createdAt = timestamp("created_at")
}

object GroupMembers : org.jetbrains.exposed.sql.Table("group_members") {
    val groupId = long("group_id").references(Groups.id)
    val userId = long("user_id").references(Users.id)
    val role = enumerationByName("role", 20, GroupRole::class).default(GroupRole.MEMBER)
    val joinedAt = timestamp("joined_at")
    override val primaryKey = PrimaryKey(groupId, userId)
}

object GroupMessages : LongIdTable("group_messages") {
    val groupId = long("group_id").references(Groups.id)
    val senderId = long("sender_id").references(Users.id)
    val body = text("body").nullable()
    val imageUrl = varchar("image_url", 500).nullable()
...
enum class StatusType { PHOTO, TEXT }

/** A status/story: photo or styled text, visible for 24 hours. Video/voice/music/GIF
 *  status types are deferred — this MVP covers the two most-used formats. */
object Statuses : LongIdTable("statuses") {
    val userId = long("user_id").references(Users.id)
    val type = enumerationByName("type", 20, StatusType::class)
    val contentUrl = varchar("content_url", 500).nullable() // photo, points at /api/media/{id}
...
object StatusViews : org.jetbrains.exposed.sql.Table("status_views") {
    val statusId = long("status_id").references(Statuses.id)
    val viewerId = long("viewer_id").references(Users.id)
    val viewedAt = timestamp("viewed_at")
    override val primaryKey = PrimaryKey(statusId, viewerId)
}
