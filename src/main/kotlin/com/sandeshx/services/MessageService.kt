...
object MessageService {
    ...
    suspend fun send(senderId: Long, receiverId: Long, body: String?, imageUrl: String?): MessageDto =
        withContext(Dispatchers.IO) {
            ...
            transaction {
                ...
                // Built directly from what we just wrote instead of a second SELECT
                // round-trip to read back the row we already know the contents of —
                // that extra query was adding latency to every single message send.
                MessageDto(
                    id = id.value,
                    senderId = senderId,
    ...
    /** Marks every unread 1:1 message addressed to [readerId] as read in one go
     *  ("mark all as read" from the chat list). Returns the distinct sender ids
     *  so the caller can push read-receipt socket events to each of them. */
    suspend fun markAllRead(readerId: Long): Set<Long> = withContext(Dispatchers.IO) {
        transaction {
            val senderIds = Messages.selectAll()
                .where { (Messages.receiverId eq readerId) and (Messages.status neq MessageStatus.READ) }
                .map { it[Messages.senderId] }
                .toSet()

            Messages.update({ (Messages.receiverId eq readerId) and (Messages.status neq MessageStatus.READ) }) {
                it[status] = MessageStatus.READ
                it[readAt] = Instant.now()
            }
            senderIds
