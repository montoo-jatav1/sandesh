...
5. Set up Firebase for push notifications — see the detailed steps above
   `FIREBASE_CREDENTIALS_JSON` in `.env.example`. Two files are involved: the
   Android app's `google-services.json` (currently a placeholder — replace it)
   and this backend's `FIREBASE_CREDENTIALS_JSON`, both from the same
   Firebase project. Without this, offline users never get a notification —
   they only see new messages once they reopen the app.
